...
public class SynchAnimationAndMedia extends Application {
		...
		scene.setOnDragDropped(new EventHandler<DragEvent>() {
			...
			public void handle(DragEvent event) {
				...
				Media media = new Media(
						"file:/E:/Video/Music/Let%20her%20go%20(Cover%20by%20Jannina%20W).mp4");
				...
				mediaPlayer.setOnPlaying(new Runnable() {
					...
					public void run() {
						animateTheEnd.stop();
						animateTheEnd.getNode().setTranslateY(
								scene.getHeight() + 40);
				...
				if (mediaView == null) {
					mediaView = MediaViewBuilder.create()
							.mediaPlayer(mediaPlayer).x(4).y(4)
							.preserveRatio(true).opacity(.85).smooth(true)
							.build();

					mediaView.fitWidthProperty().bind(
							scene.widthProperty().subtract(
									messageArea.widthProperty().add(70)));
	...
	private VBox createClosedCaptionArea(final Scene scene) {
		...
		messageArea.translateXProperty().bind(
				scene.widthProperty().subtract(152));
		messageArea.setTranslateY(20);
		closedCaption = TextBuilder.create().stroke(Color.WHITE)
				.fill(Color.YELLOW).font(new Font(15)).build();
		messageArea.getChildren().add(closedCaption);
		return messageArea;
	}

	private Node createPauseMessage(String message) {
		DropShadow dShadow = DropShadowBuilder.create().offsetX(3.5f)
				.offsetY(3.5f).build();
		Text textMessage = TextBuilder.create().text(message).x(100).y(50)
				.strokeWidth(2).stroke(Color.WHITE).effect(dShadow)
				.fill(Color.LIME).font(Font.font(null, FontWeight.BOLD, 35))
				.translateY(50).build();
		return textMessage;
	}
	...
	private Group createButtonArea(final Scene scene) {
		...
		// rounded rect
		Rectangle buttonArea = RectangleBuilder.create().arcWidth(15)
				.arcHeight(20).fill(new Color(0, 0, 0, .55)).x(0).y(0)
				.width(60).height(30).stroke(Color.rgb(255, 255, 255, .70))
				.build();
	...
	private Node createStopControl() {
		// stop audio control
		Rectangle stopButton = RectangleBuilder.create().arcWidth(5)
				.arcHeight(5).fill(Color.rgb(255, 255, 255, .80)).x(0).y(0)
				.width(10).height(10).translateX(15).translateY(10)
	...
	private Node createBackground(Scene scene) {
		// application area
		Rectangle applicationArea = RectangleBuilder.create().arcWidth(20)
				.arcHeight(20).fill(Color.rgb(0, 0, 0, .80)).x(0).y(0)
				.strokeWidth(2).stroke(Color.rgb(255, 255, 255, .70)).build();
	...
	private Slider createSlider(Scene scene) {
		Slider slider = SliderBuilder.create().min(0).max(100).value(1)
				.showTickLabels(true).showTickMarks(true).build();
		...
		scene.setOnMousePressed(new EventHandler<MouseEvent>() {
			public void handle(MouseEvent event) {
				if (!primaryStage.isFullScreen()) {
					anchorPt = new Point2D(event.getScreenX(), event
							.getScreenY());
				}
			}
		});
	...
	private Node createCloseButton(final Scene scene) {
		...
		Circle closeButton = CircleBuilder.create().centerX(5).centerY(0)
				.radius(7).fill(Color.rgb(255, 255, 255, .80)).build();
	...
	private Node createPauseControl() {
		...
		final Circle pauseButton = CircleBuilder.create().centerX(12)
				.centerY(16).radius(10).stroke(new Color(1, 1, 1, .90))
				.translateX(30).build();
		final Line firstLine = LineBuilder.create().startX(6).startY(16 - 10)
				.endX(6).endY(16 - 2).strokeWidth(3).translateX(34)
				.translateY(6).stroke(new Color(1, 1, 1, .90)).build();
		final Line secondLine = LineBuilder.create().startX(6).startY(16 - 10)
				.endX(6).endY(16 - 2).strokeWidth(3).translateX(38)
				.translateY(6).stroke(new Color(1, 1, 1, .90)).build();
	...
	private Node createPlayControl() {
		// play control
		final Arc playButton = ArcBuilder.create().type(ArcType.ROUND)
				.centerX(12).centerY(16).radiusX(15).radiusY(15)
				.startAngle(180 - 30).length(60).fill(new Color(1, 1, 1, .90))
				.translateX(40).build();
	...
	public TranslateTransition createTheEnd(Scene scene) {
		Text theEnd = TextBuilder.create().text("The End").font(new Font(40))
				.strokeWidth(3).fill(Color.WHITE).stroke(Color.WHITE).x(75)
				.build();

		TranslateTransition scrollUp = TranslateTransitionBuilder.create()
				.node(theEnd).duration(Duration.seconds(1))
				.interpolator(Interpolator.EASE_IN)
				.fromY(scene.getHeight() + 40).toY(scene.getHeight() / 2)
				.build();
		return scrollUp;
	}
